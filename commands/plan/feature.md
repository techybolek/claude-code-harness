...
Create a plan to implement the feature described below.
...
<REQUIRED whenever the feature changes anything user-visible; omit only when nothing user-visible changed. Steps the validation agent executes LIVE with the playwright-cli skill — for each step: the route to open, the element/behavior that must actually render, the action to perform, the expected observable result. These gates are equal in force to Validation Commands: a step that cannot be executed fails validation. Keep the set MINIMAL — each step is an LLM-driven browser session re-paid on every validation pass: one happy-path step per user-visible flow, plus the wiring/isolation proofs the compiler cannot see. A branch/fault-injection/edge-case step earns a place ONLY when it owns an acceptance criterion no automated test in this repo can cover — name that criterion in the step; branch coverage an automated test layer CAN express belongs in Validation Commands tests instead.>
