---
description: "Rich, interactive HTML explanation of a code change, diff, branch, or PR — background, intuition, code walkthrough, and an interactive quiz"
argument-hint: "[blank=uncommitted diff | <ref>|<range> | PR <n>]"
---

**Target:** $ARGUMENTS — if blank, explain the uncommitted working-tree diff.
...
- **Use `render.py` (installed at `~/tools/explain-diff/render.py`) instead of hand-writing HTML.**
  ...
  ```
  python ~/tools/explain-diff/render.py <spec.json>
  ```
  ...
  content spec, not the full HTML page. Run `python ~/tools/explain-diff/render.py --help` for the exact JSON
