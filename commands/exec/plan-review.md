...
- **Coordination floor (applies to all Blocking):** the plan is a coordination channel — tasks are implemented in parallel by agents that share nothing but the plan. Blocking is reserved for defects the implementers cannot self-correct: cross-task incoherence (a contract one task produces and another consumes/asserts that doesn't line up), or a defect that would ship silently past every test/gate/validation step in the plan. A defect confined to ONE task that its implementer will directly collide with while doing the work (SQL that cannot run, a gate that cannot pass, a factual claim about the codebase disproved by the first compile or test run) is a nit prefixed `ADVISORY(T{N}):` naming the affected task — implementers are instructed to resolve exactly this class in place.
