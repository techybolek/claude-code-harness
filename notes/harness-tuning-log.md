...
### Artifacts (scratchpad, session `d07c2fb6` — /tmp, lost on reboot)

- `review-flow-only.js` — standalone Validate+Code-review workflow (verbatim extraction from run-review-flow.js; args: planPath, validationCommands, skipValidation). Proven useful today; candidate for `~/.claude/workflows/`.
- `run-review-flow-revalidate.js` — one-off cache-bust resume copy (obsolete once discovery ordering is fixed).

### Proposals (pending user decision — none applied)

1. **Runtime gate, the actual fix for this class**: capture `E2E_ADMIN_STORAGE_STATE` once in the TPV2 session (playwright storage-state of a real MODIFY login), run `financial-override.spec.ts` for real — this also verifies the orchestrator's manual wiring fix, currently exactly as unverified as the fixer's wrong one. Then make the E2E a real command in this plan's Validation Commands.
2. **`run-review-flow.js` validation prompt one-liner**: "a skipped/not-run test that covers a plan acceptance criterion is a FAILURE, not a pass." Kills the green-by-skip loophole at the cheapest point.
3. **`run-review-flow.js` fixer prompt**: when a finding says UI/behavior is unreachable or not rendered, static verification is insufficient — the fixer must state what runtime evidence it could not produce, and the finding stays open rather than "fixed".
4. **Sort carried discoveries deterministically** (by source task id, then text) before embedding in implementer prompts — otherwise every resume of an in-flight run cache-busts wave 2+.
5. **Plan templates (`commands/plan/*.md`)**: Validation Commands must be executable commands; anything non-executable goes under Notes, never as a comment inside the command block.
6. **run-flow.md VALIDATION_FAILED branch**: after a manual fix, the sanctioned path is resume-with-revalidation or the review-only workflow — never `exec:review-loop`/`review-panel` (single/Claude-seat reviewers; weaker than the codex panel by the 2026-07-23 decision).
7. **YAGNI ceiling for plan-review findings** (ties to the pending plan-review restructure): test-rigor mandates (locks, concurrency proofs, fault-injection) entering via carried findings should be flagged for user decision, not silently baked into SC contracts — this one cost 2 fix rounds and still ended unresolved.
